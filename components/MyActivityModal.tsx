...
import type { ActiveCabRide, TourPackage, VehicleType, PermitRequest, SpotifyTrack, CustomTourRequest, HotelBooking, RestaurantBooking } from '../types';
...
const MyActivityModal: React.FC<MyActivityModalProps> = ({
  ...
  const hasActivity = activeCabRide || activeTourPackage || visitedPlaces.length > 0 || permitRequests.length > 0 || customTourRequests.length > 0 || hotelBookings.length > 0 || restaurantBookings.length > 0 || touristSiteBookings.length > 0 || tourBookings.length > 0;
      ...
      >
        <header className="p-4 border-b border-black/10 dark:border-white/10 flex justify-between items-center flex-shrink-0">
          <h2 id="activity-modal-title" className="text-xl font-bold text-primary">
            {showMusicPlayer ? 'In-Ride Music' : userRole && userRole !== 'tourist' ? 'Incoming Bookings' : 'My Activity'}
          </h2>
          <button
            onClick={onClose}
          ...
          {userRole && userRole !== 'tourist' ? (
            ...
            <div className="space-y-6">
              {userRole === 'restaurant_owner' && (
                <section>
                  <h3 className="text-lg font-semibold text-primary mb-3">Restaurant Reservations</h3>
                  {restaurantBookings.length > 0 ? (
                    <div className="space-y-3">
                      {restaurantBookings.map(booking => (
                        <div key={booking.id} className="glass-pane p-4">
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className="font-semibold text-primary">Booking #{booking.id}</h4>
                              <p className="text-sm text-secondary mt-1">Guest: {booking.guestName}</p>
                              <p className="text-sm text-secondary">Date: {new Date(booking.date).toLocaleDateString()} at {booking.time}</p>
                              <p className="text-sm text-secondary">{booking.adults} Adults, {booking.children} Children</p>
                            </div>
                            <span className="px-2 py-1 bg-green-500/20 text-green-500 text-xs rounded-full">Confirmed</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-secondary text-center py-4">No incoming reservations.</p>
                  )}
                </section>
              )}
              ...
              {userRole === 'hotel_owner' && (
                <section>
                  <h3 className="text-lg font-semibold text-primary mb-3">Room Bookings</h3>
                  {hotelBookings.length > 0 ? (
                        ...
                        <div key={booking.id} className="glass-pane p-4">
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className="font-semibold text-primary">Booking #{booking.id}</h4>
                              <p className="text-sm text-secondary mt-1">Guest: {booking.guestName}</p>
                              <p className="text-sm text-secondary">Check-in: {new Date(booking.checkInDate).toLocaleDateString()}</p>
                              <p className="text-sm text-secondary">Room: {booking.roomType}</p>
                            </div>
                            <span className="px-2 py-1 bg-green-500/20 text-green-500 text-xs rounded-full">Confirmed</span>
                          </div>
                        </div>
                    ...
                    <p className="text-secondary text-center py-4">No incoming bookings.</p>
                  )}
                </section>
              )}
              ...
              {userRole === 'tour_operator' && (
                <section>
                  <h3 className="text-lg font-semibold text-primary mb-3">Tour Bookings</h3>
                  {tourBookings.length > 0 ? (
                      ...
                      {tourBookings.map(booking => (
                        <div key={booking.id} className="glass-pane p-4">
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className="font-semibold text-primary">Booking #{booking.id}</h4>
                              <p className="text-sm text-secondary mt-1">Customer: {booking.customerName}</p>
                              <p className="text-sm text-secondary">Tour: {booking.tourName}</p>
                              <p className="text-sm text-secondary">People: {booking.numberOfPeople}</p>
                            </div>
                            <span className="px-2 py-1 bg-green-500/20 text-green-500 text-xs rounded-full">Confirmed</span>
                          </div>
                        </div>
                    ...
                    <p className="text-secondary text-center py-4">No tour bookings.</p>
                  )}
                </section>
              )}
            </div>
            ...
            <>
              {!hasActivity ? (
                ...
                <>
                  ...
                  {hotelBookings.length > 0 && (
                    <section>
                      <h3 className="text-lg font-semibold text-primary mb-3">Hotel Bookings</h3>
                      <div className="space-y-3">
                        {hotelBookings.map(booking => (
                          <div key={booking.id} className="glass-pane p-4">
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="font-semibold text-primary">{booking.hotelName}</h4>
                                <p className="text-sm text-secondary mt-1">
                                  Guest: {booking.guestName}
                                </p>
                                <p className="text-sm text-secondary">
                                  {booking.adults} {booking.adults === 1 ? 'adult' : 'adults'}, {booking.kids} {booking.kids === 1 ? 'kid' : 'kids'}
                                </p>
                                <p className="text-sm text-secondary">
                              ...
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${booking.status === 'confirmed' ? 'bg-green-500/20 text-green-500' :
                            ...
                            <div className="mt-3 text-xs text-secondary">
                              <p>Booked on {new Date(booking.bookingTimestamp).toLocaleDateString()}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {restaurantBookings.length > 0 && (
                    <section>
                      <h3 className="text-lg font-semibold text-primary mb-3">Restaurant Bookings</h3>
                      <div className="space-y-3">
                        {restaurantBookings.map(booking => (
                          <div key={booking.id} className="glass-pane p-4">
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="font-semibold text-primary">{booking.restaurantName}</h4>
                                <p className="text-sm text-secondary mt-1">
                                  Guest: {booking.guestName}
                                </p>
                                <p className="text-sm text-secondary">
                                ...
                                <p className="text-sm text-secondary">
                                  {booking.adults} {booking.adults === 1 ? 'adult' : 'adults'}, {booking.children} {booking.children === 1 ? 'child' : 'children'}
                                </p>
                                <p className="text-sm text-secondary">
                                ...
                                {booking.mealType && (
                                  <p className="text-sm text-secondary">
                                    Meal: {booking.mealType.charAt(0).toUpperCase() + booking.mealType.slice(1)}
                                  </p>
                                )}
                              </div>
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${booking.status === 'confirmed' ? 'bg-green-500/20 text-green-500' :
                            ...
                            {/* QR Code for Restaurant Booking */}
                            <div className="mt-3 flex justify-center">
                              <div className="bg-white p-2 rounded-lg">
                                <img
                                  src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(JSON.stringify({
                                    ...
                                    bookingId: `RESTAURANT-${booking.id}`,
                                    timestamp: booking.bookingTimestamp
                                  }))}`}
                                  alt="Restaurant Booking QR Code"
                                  className="w-24 h-24"
                                />
                              </div>
                            </div>
                  ...
                  {touristSiteBookings.length > 0 && (
                    <section>
                      <h3 className="text-lg font-semibold text-primary mb-3">Tourist Site Bookings</h3>
                      <div className="space-y-3">
                        {touristSiteBookings.map(booking => (
                          <div key={booking.id} className="glass-pane p-4">
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="font-semibold text-primary">{booking.siteName}</h4>
                                <p className="text-sm text-secondary mt-1">
                                ...
                                <p className="text-sm text-secondary">
                                  Ticket Type: {booking.ticketType.charAt(0).toUpperCase() + booking.ticketType.slice(1)}
                                </p>
                              </div>
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${booking.status === 'confirmed' ? 'bg-green-500/20 text-green-500' :
                                booking.status === 'completed' ? 'bg-blue-500/20 text-blue-500' :
                            ...
                            {/* QR Code for Tourist Site Booking */}
                            <div className="mt-3 flex justify-center">
                              <div className="bg-white p-2 rounded-lg">
                                <img
                                  src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(JSON.stringify({
                                    ...
                                    bookingId: `SITE-${booking.id}`,
                                    timestamp: booking.bookingTimestamp
                                  }))}`}
                                  alt="Tourist Site Booking QR Code"
                                  className="w-24 h-24"
                                />
                              </div>
                            </div>
                      ...
                      <div className="space-y-3">
                        {tourBookings.map(booking => (
                          <div key={booking.id} className="glass-pane p-4">
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="font-semibold text-primary">{booking.tourName}</h4>
                                <p className="text-sm text-secondary mt-1">
                              ...
                              <div className="bg-white p-2 rounded-lg">
                                <img
                                  src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(JSON.stringify({
                                    ...
                                    bookingId: `TOUR-${booking.id}`,
                                    timestamp: booking.bookingTimestamp
                                  }))}`}
                                  alt="Tour Booking QR Code"
                                  className="w-24 h-24"
                                />
                              </div>
                            </div>
                  ...
                  {customTourRequests.length > 0 && (
                    <div className="mt-6">
                      <h3 className="text-lg font-bold text-primary mb-3">Custom Tour Requests</h3>
                      <div className="space-y-3">
                        {customTourRequests.map(request => (
                          <div key={request.id} className="glass-pane p-4">
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="font-semibold text-primary">Custom Tour Request</h4>
                                <p className="text-sm text-secondary mt-1">
                                  {request.adults} {request.adults === 1 ? 'adult' : 'adults'}, {request.children} {request.children === 1 ? 'child' : 'children'}
                                </p>
                                <p className="text-sm text-secondary">
                                  Transport: {request.transportMode}, Tier: {request.packageTier}
                                </p>
                              </div>
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${request.status === 'pending' ? 'bg-yellow-500/20 text-yellow-500' :
                            ...
                            <div className="mt-3 text-xs text-secondary">
                              <p>Requested on {new Date(request.timestamp).toLocaleDateString()}</p>
                            </div>
                          </div>
                        ))}
                      </div>
