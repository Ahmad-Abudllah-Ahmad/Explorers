...
const LahoreHubIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 21h16.5M9 3v18m6-18v18M9 6.75h6M9 12h6m-6 5.25h6" />
...
const GridButton: React.FC<GridButtonProps> = ({ onClick, disabled, label, icon, imageUrl, notificationCount, title }) => (
  <div
    ...
    className={`relative aspect-square flex flex-col items-center justify-center p-1 text-center rounded-2xl transition-all duration-300 transform focus:outline-none focus:ring-2 focus:ring-[color:var(--accent-color)] focus:ring-offset-2 dark:focus:ring-offset-black 
            ${disabled
        ? 'opacity-60 cursor-not-allowed saturate-50'
        : 'cursor-pointer hover:scale-105 active:scale-95 shadow-md hover:shadow-lg'
      }`}
    title={title}
  >
    <div
      className="absolute inset-0 bg-cover bg-center rounded-2xl -z-10"
      style={{ backgroundImage: `url(${imageUrl})` }}
    />
    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-black/20 rounded-2xl -z-10" />
    <div className="absolute inset-0 ring-1 ring-inset ring-white/10 rounded-2xl -z-10" />

    <div className="text-white drop-shadow-lg">
      {icon}
    </div>
    <span className="text-xs font-semibold text-white mt-1 drop-shadow-lg">{label}</span>

    {notificationCount && notificationCount > 0 && (
      <span className={`absolute top-1 right-1 flex items-center justify-center h-4 min-w-[1rem] px-1 rounded-full bg-red-500 text-white text-[0.6rem] font-bold ring-1 ring-white/50`} title={`${notificationCount} active items`}>
        {notificationCount}
      </span>
    )}
  </div>
...
const ExpandedCategoryBanner = ({ category, onBack, onViewResults, onResetSearch, isActive }: { category: any, onBack: () => void, onViewResults: () => void, onResetSearch: () => void, isActive: boolean }) => {
  // Don't show banner for special categories
  const specialCategories = ["LahoreHub"];
  if (specialCategories.includes(category.label)) {
    return null;
  }
...
const CategoryButtons: React.FC<CategoryButtonsProps> = ({ onSearch, onCabBooking, onScanRequest, onCommunityChatRequest, onTranslatorRequest, onTourPackagesRequest, onActivityRequest, onPermitRequest, onHikingRequest, onLahoreHubRequest, disabled, onViewResults, onResetSearch, activeCabRide, visitedPlacesCount, permitRequestsCount, activeCategory, setActiveCategory }) => {
  ...
  // Hide categories when window is resized to desktop size
  useEffect(() => {
    const handleResize = () => {
  ...
  const categories = [
    ...
    // New LahoreHub button
    { label: "Make Custom Tour Creation", icon: <LahoreHubIcon />, action: onLahoreHubRequest, imageUrl: "https://images.unsplash.com/photo-1598965391050-50d4bcb3e9f2?q=80&w=800&auto=format&fit=crop", description: "Create your personalized tour with verified providers.", actionLabel: "Create Tour" },
  ];
  ...
  const handleGridButtonClick = (category: (typeof categories)[0]) => {
    ...
    // Check if it's a special category that should trigger its action immediately
    const specialCategories = ["LahoreHub"]; // Add LahoreHub to special categories
    const isSearchCategory = ["Restaurants", "Hotels", "Tourist Sites"].includes(category.label);

    if (isSearchCategory) {
      category.action();
    } else if (specialCategories.includes(category.label)) {
      // For special categories like LahoreHub, trigger the action immediately
      category.action();
      // Don't set the active category for special categories
  ...
  return (
    <div className="mt-4 pt-4 border-t border-black/10 dark:border-white/10">
      ...
      <div className={`md:hidden mb-4 transition-all duration-500 ease-in-out ${activeCategory ? 'opacity-0 h-0 overflow-hidden mb-0' : 'opacity-100 h-auto'}`}>
        <button
          onClick={toggleCategories}
          className="w-full flex items-center justify-between p-3 glass-button-primary rounded-2xl"
        >
          <span className="font-semibold">Explore Categories</span>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className={`h-5 w-5 transition-transform duration-300 ${showCategories ? 'rotate-180' : ''}`}
            viewBox="0 0 20 20"
            fill="currentColor"
          >
      ...
      <div className={`relative transition-all duration-500 ease-in-out ${activeCategory ? 'h-[45vh] min-h-[320px] md:h-[400px]' : ''}`}>
        {/* Grid View */}
        <div className={`transition-all duration-300 ease-in-out ${activeCategory ? 'opacity-0 scale-95 invisible' : 'opacity-100 scale-100 visible'}`}>
          <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-3 ${showCategories ? 'block' : 'hidden md:grid'}`}>
            {categories.map((cat) => {
        ...
        {activeCategoryData && (
          <ExpandedCategoryBanner
            ...
            onBack={() => {
              setActiveCategory(null);
              // Reset search results to show pre-made tours
              onResetSearch();
            }}
